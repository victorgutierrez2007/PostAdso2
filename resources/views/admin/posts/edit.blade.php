        ...
        <div class="max-w-4xl mx-auto">
            ...
            <div class="mb-8">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-4xl font-bold text-gray-900 dark:text-white">{{ __('Editar Post') }}</h1>
                        <p class="text-gray-600 dark:text-gray-400 mt-1">{{ __('Actualizar información del artículo') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-md border border-gray-200 dark:border-zinc-700 p-8">
                <form action="{{ route('admin.posts.update', $post) }}" method="POST" class="space-y-6" id="post-form">
                    @csrf
                    @method('PUT')

                    {{-- ID (solo lectura) --}}
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-gray-50 dark:bg-zinc-900/50 rounded-lg border border-gray-200 dark:border-zinc-700 text-sm">
                        <div>
                            <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">ID</p>
                            <p class="font-bold text-gray-900 dark:text-white">{{ $post->id }}</p>
                        </div>
                        <div>
                            <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">User ID</p>
                            <p class="font-bold text-gray-900 dark:text-white">{{ $post->user_id }} <span class="font-normal text-gray-500">({{ $post->user->name ?? '-' }})</span></p>
                        </div>
                        <div>
                            <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">Creado</p>
                            <p class="font-bold text-gray-900 dark:text-white">{{ $post->created_at?->format('d/m/Y H:i') }}</p>
                        </div>
                        <div>
                            <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">Actualizado</p>
                            <p class="font-bold text-gray-900 dark:text-white">{{ $post->updated_at?->format('d/m/Y H:i') }}</p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 mb-2">Identificador único en la URL. Se auto-genera si lo dejas vacío.</p>
                        <input
                            type="text"
                            id="slug"
                            name="slug"
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            name="excerpt"
                            rows="3"
                            placeholder="{{ __('Resumen breve del artículo') }}"
                    ...
                    <div>
                        ...
                        @if($post->image_path)
                            <div class="mb-3 flex items-center gap-3 p-3 bg-gray-50 dark:bg-zinc-900/50 rounded-lg border border-gray-200 dark:border-zinc-700">
                                <img
                                    src="{{ Str::startsWith($post->image_path, ['http://','https://']) ? $post->image_path : asset($post->image_path) }}"
                                    alt="{{ $post->title }}"
                                    class="w-16 h-16 rounded-lg object-cover border border-gray-200 dark:border-zinc-600 flex-shrink-0"
                                >
                                <div class="min-w-0">
                                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-0.5">Imagen actual</p>
                                    <p class="text-xs text-gray-600 dark:text-gray-300 font-mono break-all">{{ Str::limit($post->image_path, 60) }}</p>
                                </div>
                            </div>
                        @endif
                        <input
                            ...
                            id="image_path"
                            name="image_path"
                            value="{{ old('image_path', $post->image_path) }}"
